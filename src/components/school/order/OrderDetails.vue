...
export default {
  ...
  async mounted() {
    ...
    let self = this;
    await OrderApi.getOrderDetailsSchool(payload).then((e) => {
      self.order = e.data[0];
      let env = process.env.VUE_APP_API_URL;
      // console.log("base url", env);
      let img = env.replace(`.org/api`, "");
      // console.log("imgpath", img);
      self.orderStatus =
        e.data[0].status === null ? "processing" : e.data[0].status;
      self.order.product.image_path =
        img + ".org/" + e.data[0].product.image_path;
    });
    this.loading = false;
  },
  methods: {
    async orderStatusChnage() {
      ...
      this.loading = true;
      await OrderApi.changeOrderStatusSchool(payload).then(() => {
        self.loading = false;
        self.$router.push(`/school/order/list`);
      });
    },
  },
};
