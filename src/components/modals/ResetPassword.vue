<template>
  <modal
    ...
    :width="deviceWidth"
    height="auto"
    classes="u-modal reset-password"
    scrollable
  >
    <u-card class="u-modal-card u-px-24 u-pt-24 u-pb-15">
      <div class="reset-password__cross" @click="hideModal">
        <svg v-svg symbol="icon-cross" size="0 0 72 72" />
      </div>
      <slot name="title">
        <div class="u-text-h2 u-text-center">Forgot Password?</div>
      </slot>
      ...
      <ValidationObserver v-slot="{ handleSubmit }">
        <form
          ...
          @submit.prevent="() => handleSubmit(() => resetPassword(login))"
        >
          <ValidationProvider
            rules="required"
          ...
          <div class="reset-password__submit">
            <UBtn color="primary" size="x-large" type="submit" width="262">
              Submit
            </UBtn>
          </div>
        </form>
