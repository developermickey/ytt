...
export default {
  ...
  mutations: {
    ...
    TOGGLE_WORD(state, wordId) {
      state.words = state.words.map((item) => {
        if (item.id === wordId) item.isKnown = !item.isKnown;
        return item;
      });

      if (state.wordsToggled.indexOf(wordId) !== -1) {
        state.wordsToggled = state.wordsToggled.filter(
          (item) => item !== wordId
        );
      } else {
    ...
    CHANGE_WORD_STATUS(state, { wordId, status }) {
      ...
      state.words = state.words.map((item) => {
        if (item.id === wordId) item.status = status;
  ...
  actions: {
    ...
    // updateMultipleStudentsWords({ commit, getters, state }, payload) {
    //   let url = ``;
